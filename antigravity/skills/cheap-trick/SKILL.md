...
- **It hit the ceiling of its tier** — right brief, wrong answer, or 5× the turns → go up one tier for that stage. Do not retry the cheapest tier on something the cheapest tier already failed.
...
DELEGATE — read family A (4 pages + imports), return rules + citations   → mid tier
DELEGATE — read family B                                                 → mid tier
DELEGATE — read family C                                                 → mid tier
DELEGATE — read family D                                                 → mid tier
...
Footer for that run: `Cheap Trick: 7 stages → 4 delegated (mid tier ×4), 2 kept, 1 inline`.
...
│ CHEAP TRICK  5 stages → 1 delegated (cheapest tier), 3 kept, 1 inline
